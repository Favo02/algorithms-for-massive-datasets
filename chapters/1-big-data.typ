...
We can describe the computation using a *Computation Graph*, where each node represents a computation that can start only when the _predecessors_ are finished (when the input for the node is ready).
The complexity of that system is the number of _things_ that _travel_ that graph (tipically the number of tuples).

To compute this count, we can simply sum of _size of all inputs_ to nodes.
The only thing we does _not_ get counted with this approach is the _final output_.
But the cost of the final output is _negligible_ (as we cannot reduce it, it is the output!).

#note[
  That's *not* true in Hadoop DFS, as the input for the first computation is handled _locally_ on the same machine the chunk is stored, so it does _not_ travel the network.
...
/ Double join (multi-way): $R(A, B) join S(B, C) join T(C, D), quad |R| = r, |S| = s, |T| = t$
  ...
  But thats a *multi-variable* function, so we can't calculate "normal" derivative, we need partial derivatives for each variable, forming a *gradient* $nabla$ and setting it to zero.
  ...
  #example[
    ...
    #warning[
      If we use a lot of nodes ($> 961$) we perform worse than having less nodes, because the communication overhead is too big.
    ]
  ]
