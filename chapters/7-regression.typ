...
This is incredibly simple, but it is fast and often works remarkably well.
...
#informally[
  Data that cannot be linearly separated in 2D (like a circle of red dots surrounded by blue dots), can be separated in a 3D space (using height?). Now, you can easily slide a flat sheet of metal (a linear plane) right between them!
]
...
Let's reformulate this cleanly using vectors and matrices.
...
Everything boils down to the dimensionality of the data ($d$) and the number of data points ($n$):
...
Since I just set up my clean PC as a dedicated home server, the last thing I want is to stress-test it with a massive local matrix multiplication until it overheats and shuts down again!
...
A perfect classifier yields L=0. The value of this macro loss function directly represents the total count of misclassified examples.
...
This is a continuous, convex function. So we will minimize the log-loss:
$ w^* = arg min_w sum_(j=1)^n ell_(log) (y^((j)) (w dot x^((j)))) $

We need to compute the gradient! First of all, we need the derivative of the log-loss function (applying the chain rule):
...
Then compute the partial derivatives with respect to wk:
$ (partial ell) / (partial w_k) = sum_(j=1)^n -(1-1/(1+e^(-w dot x^((j)) y^((j))))) dot d / (d w_k) (w dot x^((j)) y^((j))) $

Since d/(dwk)(wdotx((j))y((j)))=y((j))xk((j)), we get:
...
We use a Sigmoid function on beta:
...
- On the diagonal, it is still random, but with a different probability on the coin flip.